...
public abstract class AbstractConnection
{
   ...
   public void addFallback(String objectprefix, DBusInterface object) throws DBusException
   {
      ...
      if (!objectprefix.matches(OBJECT_REGEX)||objectprefix.length() > MAX_NAME_LENGTH) 
         throw new DBusException(gt_("Invalid object path: ")+objectprefix);
         ExportedObject eo = new ExportedObject(object, weakreferences);
   ...
   */
   ...
   private void handleMessage(final MethodCall m) throws DBusException
   {
      ...
      if (null == o) {
         ...
         if (null == eo) {
            try {
               queueOutgoing(new Error(m, new DBus.Error.UnknownObject(m.getPath()+gt_(" is not an object provided by this process.")))); 
         ...
         if (null == meth) {
            try {
               queueOutgoing(new Error(m, new DBus.Error.UnknownMethod(MessageFormat.format(gt_("The method `{0}.{1}' does not exist on this object."), new Object[] { m.getInterface(), m.getName() })))); 
            } catch (DBusException DBe) {}
            return;
         }
         o = eo.object.get();
      ...
      { 
         ...
         { 
            ...
            try {
               ...
               if (EXCEPTION_DEBUG && Debug.debug) Debug.print(Debug.ERR, e);
               try {
                  conn.queueOutgoing(new Error(m, new DBus.Error.UnknownMethod(gt_("Failure in de-serializing message: ")+e))); 
               } catch (DBusException DBe) {} 
               return;
            }
            ...
            try { 
               ...
               if (EXCEPTION_DEBUG && Debug.debug) Debug.print(Debug.ERR, e);
               try { 
                  conn.queueOutgoing(new Error(m, new DBusExecutionException(MessageFormat.format(gt_("Error Executing Method {0}.{1}: {2}"), new Object[] { m.getInterface(), m.getName(), e.getMessage() })))); 
               } catch (DBusException DBe) {}
            } 
         }
      });
   ...
   private void handleMessage(final MethodReturn mr)
   {
      ...
      if (null != m) {
         ...
         try {
            queueOutgoing(new Error(mr, new DBusExecutionException(gt_("Spurious reply. No message with the given serial id was awaiting a reply.")))); 
         } catch (DBusException DBe) {}
   }
   protected void sendMessage(Message m)
   {
      try {
			if (!connected) throw new NotConnected(gt_("Disconnected"));
         if (m instanceof DBusSignal) 
         ...
         if (m instanceof MethodCall) {
            if (0 == (m.getFlags() & Message.Flags.NO_REPLY_EXPECTED))
               if (null == pendingCalls) 
                  ((MethodCall) m).setReply(new Error("org.freedesktop.DBus.Local", "org.freedesktop.DBus.Local.Disconnected", 0, "s", new Object[] { gt_("Disconnected") }));
         ...
         if (m instanceof MethodCall && e instanceof DBusExecutionException) 
            ...
            try {
               ...
               ((MethodCall)m).setReply(new Error(m, new DBusExecutionException(gt_("Message Failed to Send: ")+e.getMessage())));
            } catch (DBusException DBe) {}
         else if (m instanceof MethodReturn)
            try {
   ...
   {
      if (!connected) throw new NotConnected(gt_("No transport present"));
      Message m = null;
      try {
