...
public class DBusDaemon extends Thread
{
   ...
   public class DBusServer extends Thread implements DBus, DBus.Introspectable, DBus.Peer
   {
      ...
      public String Hello() 
      {
         ...
         synchronized (c) {
            if (null != c.unique) 
               throw new org.freedesktop.DBus.Error.AccessDenied(gt_("Connection has already sent a Hello message"));
      ...
      private void handleMessage(Connstruct c, Message m) throws DBusException
      {
         ...
         try {
            ...
            try {
               ...
               send(c,new org.freedesktop.dbus.Error("org.freedesktop.DBus", c.unique, "org.freedesktop.DBus.Error.GeneralError", m.getSerial(), "s", gt_("An error occurred while calling ")+m.getName()));
            }
         } catch (NoSuchMethodException NSMe) {
            send(c,new org.freedesktop.dbus.Error("org.freedesktop.DBus", c.unique, "org.freedesktop.DBus.Error.UnknownMethod", m.getSerial(), "s", gt_("This service does not support ")+m.getName()));
         }
						...
						for (WeakReference<Connstruct> wc: wcs) {
							...
							if (null != c) {
								...
								if (null == c.unique 
									...
									send(c,new Error("org.freedesktop.DBus", null, "org.freedesktop.DBus.Error.AccessDenied", m.getSerial(), "s", gt_("You must send a Hello message")));
								} else {
									try {
										if (null != c.unique) m.setSource(c.unique);
									} catch (DBusException DBe) {
										if (Debug.debug && AbstractConnection.EXCEPTION_DEBUG) Debug.print(Debug.ERR, DBe);
										send(c,new Error("org.freedesktop.DBus", null, "org.freedesktop.DBus.Error.GeneralError", m.getSerial(), "s", gt_("Sending message failed")));
									}
									...
									if ("org.freedesktop.DBus".equals(m.getDestination())) {
										...
										if (m instanceof DBusSignal) {
											...
											if (null == dest) {
												send(c, new Error("org.freedesktop.DBus", null, "org.freedesktop.DBus.Error.ServiceUnknown", m.getSerial(), "s", MessageFormat.format(gt_("The name `{0}' does not exist"), new Object[] { m.getDestination() })));
											} else
												send(dest, m);
										}
									}
