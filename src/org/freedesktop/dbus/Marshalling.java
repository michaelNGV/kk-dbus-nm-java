...
public class Marshalling
{
   ...
   public static String[] recursiveGetDBusType(Type c, boolean basic, int level) throws DBusException
   {
      ...
      if (basic && !(c instanceof Class))
         throw new DBusException(c+gt_(" is not a basic type"));
      ...
      if (c instanceof TypeVariable) out[level].append((char) Message.ArgumentType.VARIANT);
         ...
         if (s.length != 1) throw new DBusException(gt_("Multi-valued array types not permitted"));
         ...
         if (null == newtypes) throw new DBusException(gt_("Serializable classes must implement a deserialize method"));

         String[] sigs = new String[newtypes.length];
         for (int j = 0; j < sigs.length; j++) {
            String[] ss = recursiveGetDBusType(newtypes[j], false, level+1);
            if (1 != ss.length) throw new DBusException(gt_("Serializable classes must serialize to native DBus types"));
            sigs[j] = ss[0];
         }
         return sigs;
      } 
         ...
         if (p.getRawType().equals(Map.class)) {
            ...
            try {
               ...
               throw new DBusException(gt_("Map must have 2 parameters"));
            }
            out[level].append('}');
         }
            ...
            throw new DBusException(gt_("Exporting non-exportable parameterized type ")+c);
      }
         ...
         throw new DBusException(gt_("Exporting non-exportable type ")+c);
      }
         ...
         for (; i < dbus.length() && (-1 == limit || limit > rv.size()); i++) 
            switch(dbus.charAt(i)) {
               ...
               case Message.ArgumentType.DICT_ENTRY1:
                  ...
                  break;
               default:
                  throw new DBusException(MessageFormat.format(gt_("Failed to parse DBus type signature: {0} ({1})."), new Object[] { dbus, dbus.charAt(i) }));
            }
         return i;
      } catch (IndexOutOfBoundsException IOOBe) {
         if (AbstractConnection.EXCEPTION_DEBUG && Debug.debug) Debug.print(Debug.ERR, IOOBe);
         throw new DBusException(gt_("Failed to parse DBus type signature: ")+dbus);
      }
   }
   /**
   ...
   static Object[] deSerializeParameters(Object[] parameters, Type[] types, AbstractConnection conn) throws Exception
   {
      ...
      for (int i = 0; i < parameters.length; i++) {
         ...
         if (i >= types.length) {
            ...
            throw new DBusException(gt_("Error deserializing message: number of parameters didn't match receiving signature"));
         }
         if (null == parameters[i]) continue;
            ...
            for (Method m: dsc.getDeclaredMethods()) 
               if (m.getName().equals("deserialize")) {
                  ...
                  try {
                     ...
                     if (AbstractConnection.EXCEPTION_DEBUG && Debug.debug) Debug.print(Debug.ERR, AIOOBe);
                     throw new DBusException(MessageFormat.format(gt_("Not enough elements to create custom object from serialized data ({0} < {1})."), 
