...
class RemoteInvocationHandler implements InvocationHandler
{
   ...
   public static Object convertRV(String sig, Object[] rp, Method m, AbstractConnection conn) throws DBusException
   {
      ...
      if (null == rp) { 
         ...
         else throw new DBusExecutionException(gt_("Wrong return type (got void, expected a value)"));
      } else {
         try { 
            ...
            throw new DBusExecutionException(MessageFormat.format(gt_("Wrong return type (failed to de-serialize correct types: {0} )"), new Object[] { e.getMessage() }));
         }
      }
      ...
      switch (rp.length) {
         ...
         default:
            ...
            if (!Tuple.class.isAssignableFrom(c))
               throw new DBusExecutionException(gt_("Wrong return type (not expecting Tuple)"));
   ...
   public static Object executeRemoteMethod(RemoteObject ro, Method m, AbstractConnection conn, int syncmethod, CallbackHandler callback, Object... args) throws DBusExecutionException
   {
      ...
      if (ts.length > 0) try {
         ...
         throw new DBusExecutionException(gt_("Failed to construct D-Bus type: ")+DBe.getMessage());
      }
      MethodCall call;
      byte flags = 0;
      ...
      try {
         ...
         throw new DBusExecutionException(gt_("Failed to construct outgoing method call: ")+DBe.getMessage());
      }
      if (null == conn.outgoing) throw new NotConnected(gt_("Not Connected"));
      ...
      if (null == reply) throw new DBus.Error.NoReply(gt_("No reply within specified time"));
